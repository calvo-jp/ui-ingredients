...
<TagsInput.Root class="mx-auto max-w-[20rem]">
  {#snippet children(context)}
    ...
    <div
      class="flex min-h-12 flex-wrap gap-2 rounded border border-neutral-700 p-2"
    >
      <TagsInput.Control class="flex flex-wrap gap-2">
        {#each context.value as value, index}
          <TagsInput.Item {index} {value} class="h-7">
            <TagsInput.ItemPreview
              class="flex h-full shrink-0 items-center gap-1 rounded border border-neutral-700 bg-neutral-800/50 px-1.5 text-sm leading-[0.875rem]"
            >
